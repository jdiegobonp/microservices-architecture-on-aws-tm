generate_hcl "_tm_security-group.tf" {

  content {
    # Security Group for Client ALB
    resource "aws_security_group" "client_alb" {
        name_prefix = "${global.project_tag}-ecs-client-alb"
    ...
    resource "aws_security_group" "ecs_client_service" {
        name_prefix = "${global.project_tag}-ecs-client-service"
    ...
    resource "aws_security_group" "ecs_fruits_service" {
        name_prefix = "${global.project_tag}-ecs-fruits-service"
    ...
    resource "aws_security_group" "ecs_vegetables_service" {
        name_prefix = "${global.project_tag}-ecs-vegetables-service"
    ...
    resource "aws_security_group" "database" {
        name_prefix = "${global.project_tag}-database"
    ...
    resource "aws_security_group" "consul_server" {
        name_prefix = "${global.project_tag}-consul-server"
    ...
    resource "aws_security_group" "consul_server_alb" {
        name_prefix = "${global.project_tag}-consul-server-alb"
    ...
    resource "aws_security_group_rule" "consul_server_alb_allow_80" {
        ...
        from_port         = 80
        to_port           = 80
        cidr_blocks       = flatten([global.consul_server_allowed_cidr_blocks, [global.vpc_cidr]])
        description       = "Allow HTTP traffic."
    }
    ...
    resource "aws_security_group" "acl_controller" {
        name_prefix = "${global.project_tag}-acl-controller-"
    ...
    resource "aws_security_group" "consul_client" {
        name_prefix = "${global.project_tag}-consul-client-"
